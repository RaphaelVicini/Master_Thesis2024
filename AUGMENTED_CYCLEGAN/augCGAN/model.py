...
class CycleGAN(object):
    ...
    def __init__(self, opt, testing=False):
        ...
        self.netD_A = networks.define_D_A(input_nc=opt.input_nc,
                                          ...
                                          norm=opt.norm, use_sigmoid=opt.use_sigmoid, num_D=opt.num_D,
                                          gpu_ids=opt.gpu_ids)
    ...
    def train_instance(self, real_A, real_B, *args):
        ...
        loss_cycle_A = 10 * self.criterionCycle(rec_A, real_A)

        ##### B -> A -> B cycle loss
        rec_B = self.netG_A_B.forward(fake_A)
        loss_cycle_B = 10 * self.criterionCycle(rec_B, real_B)

        ##### Generation and Encoder optimization
        loss_G = loss_G_A + loss_G_B + loss_cycle_A * self.opt.lambda_A + loss_cycle_B * self.opt.lambda_B
    ...
    def generate_cycle(self, real_A, real_B, *args):
        fake_B = self.netG_A_B.forward(real_A)
        fake_A = self.netG_B_A.forward(real_B)
        rec_A = self.netG_B_A.forward(fake_B)
        rec_B = self.netG_A_B.forward(fake_A)
...
class AugmentedCycleGAN(object):
    ...
    def __init__(self, opt, testing=False):
        ...
        self.num_D = opt.num_D
        self.num_D = 2
        self.loss_names = ['D_A', 'G_A', 'Cyc_A', 'Cyc_z_B', 'KLD_z_B', 'D_B', 'G_B', 'Cyc_B', 'D_z_B', 'P_t_A', 'P_f_A', 'P_t_B', 'P_f_B' ]
