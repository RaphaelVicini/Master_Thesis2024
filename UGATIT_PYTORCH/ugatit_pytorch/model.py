...
class Discriminator(nn.Module):
    def __init__(self, n_layers=5):
        ...
        model = [nn.ReflectionPad2d(1),
                 nn.utils.spectral_norm(nn.Conv2d(1, 64, 4, 2, 0, bias=True)),
...
class Generator(nn.Module):
    def __init__(self, image_size=256):
        ...
        down_layer = [
            ...
            nn.Conv2d(1, 64, 7, 1, 0, bias=False),
...
#model = Generator(image_size=256)
model = Discriminator()


model.to(device)

# Initialisation de TensorBoard SummaryWriter
log_dirzzz = r"C:\Users\rapha\OneDrive\Bureau\Discriminator_UGATIT"  # Chemin du dossier pour enregistrer les logs
writer = SummaryWriter(log_dir=log_dirzzz)

# Ajout du modèle à TensorBoard
dummy_input = torch.rand(1, 1, 256, 256).to(device)  # Assurez-vous que la taille de l'entrée est correcte pour votre modèle
#writer.add_image(dummy_input)
