...
class CustomPatientDatasetTESTING(tio.SubjectsDataset):
    def __init__(self, root_dir, subset=None):
        ...
        modality = 'T2' #changé car j'ai que des T2 pour le test

        # Définir les transformations ici, potentiellement différentes selon le subset

        transform = tio.Compose([
            tio.ToCanonical(),
            tio.Resize((512, 512, 128)),
            tio.Clamp(out_min=-150, out_max=1000, exclude=['irm']),
            tio.RescaleIntensity(out_min_max=(-1, 1)),  # exclude=['ct']
        ...
        for patient_number in sorted(os.listdir(root_dir), key=lambda x: int(x)):
            ...
            irm_path = os.path.join(patient_dir,'MRI', f'{patient_number}_valid_slices.nii')
            if os.path.exists(irm_path):
                ct_path = os.path.join(patient_dir, 'CT', f'{patient_number}_valid_slices.nii')
...
class DataLoader(object):
    def __init__(self, opt, subset, unaligned, batchSize,
        ...
        self.opt = opt
        self.dataset = Mr2Ct(opt, subset, unaligned, fraction, load_in_mem)
